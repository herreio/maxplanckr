...
from ..utils.paths import LOG_DIR, EXTDATA_DIR, CTX_DIR, OUS_DIR, GRAPH_DIR
...
def routine():
    ...
    log = open(os.path.join(LOG_DIR, "graph_sel_contexts_mpis.log"), "w+")
    sys.stdout = log
    ...
    utils.write_csv(os.path.join(
        GRAPH_DIR, 'sel--ous_nodes-ctx.csv'), institutes)
    ...
    utils.write_csv(os.path.join(
        GRAPH_DIR, 'sel--ous_ctx_edges.csv'), institutes_contexts)
    ...
    utils.write_csv(os.path.join(GRAPH_DIR, 'sel--ctx_nodes.csv'), contexts)
